...
# ---------------------------------------------------------------------------
# Простая аналитика (в памяти, без БД)
# ---------------------------------------------------------------------------
analytics: dict[str, int] = {
    "started": 0,   # сколько раз нажали "🔥 Пройти тест"
}
...
QUESTIONS = [
    ...
    },
    {
        ...
        "options": [
            ("Удалить это дерьмо", 0),
            ("Сомнительно, но океййй", 1),
            ("Нравится", 2),
            ("Выгляжу охуенно", 3),
        ],
    },
    ...
    {
        "category": "B",
        "text": "Когда тебе нужно выглядеть максимально хорошо, насколько ты уверен, что сможешь собрать сильный образ?",
        "options": [
            ("Вообще не уверен", 0),
            ("Скорее не уверен", 1),
            ("Скорее уверен", 2),
...
def get_level_text(total_score: int) -> tuple[str, str]:
    ...
    if 0 <= total_score <= 15:
        ...
        return (
            ...
            "Но пока нет системы.\n\n"
            "Иногда образ получается нонрмальным, но чаще всего нет понимания, как это все сочетать грамотно .\n\n"
        ...
        return (
            ...
            "Ты не выглядишь уже как нормис, но стиль пока не стал твоим "
...
WEAK_ZONE_TEXTS = {
    ...
    "D": (
        ...
        "Проблема не в том, что ты скучный.\n\n"
...
WELCOME_TEXT = (
    ...
    "• помогает ли внешний вид привлекать внимание\n"
    "• есть ли у тебя вообще свой стиль\n\n"
    "За 3 минуты ты узнаешь:\n"
    "✓ свой уровень\n"
    "✓ свою главную зону роста\n"
    "✓ что именно сейчас мешает тебе выглядеть сильнее\n\n"
    "Перед началом — одно важное правило.\n"
    "Отвечай честно.\n"
    "Не выбирай вариант ответа, которым хочешь быть.\n"
    "Выбирай вариант, который максимально отражает тебя сегодня.\n\n"
    "В конце обязательно сделай скриншот результата.\n"
    "В ближайшие дни я покажу, как прокачивать каждую из этих зон."
)
...
def get_start_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Пройти тест", callback_data="start_test")]
        ]
    )


async def send_welcome_screen(chat_id: int):
    """
    Отправляет единый стартовый экран: ФОТО + ТЕКСТ (caption) + КНОПКА.

    Если файла assets/welcome.jpg нет — отправляет тот же текст обычным
    сообщением (бот не должен падать из-за отсутствующей картинки).
    """
    ...
    if os.path.exists(WELCOME_IMAGE_PATH):
        ...
        # Картинки нет — отправляем тот же текст обычным сообщением
        logger.warning(
            "Файл %s не найден — отправляю приветствие без фото.",
            WELCOME_IMAGE_PATH,
        )
...
async def send_question(chat_id: int, question_index: int, edit_message: Message | None = None):
    ...
    Если edit_message — это сообщение с фото (например, стартовый экран
    с картинкой), редактировать его текст нельзя (у фото-сообщения текста
    нет, только caption). В этом случае удаляем старое сообщение и
    отправляем новое.
    """
    question = QUESTIONS[question_index]
    ...
    if edit_message is not None and edit_message.text is not None:
        # Обычное текстовое сообщение — можно редактировать
        await edit_message.edit_text(progress_text, reply_markup=keyboard)
    else:
        # Либо нет сообщения для редактирования, либо это сообщение с фото
        # (caption, а не text) — его нельзя отредактировать через edit_text
        if edit_message is not None:
            try:
                await edit_message.delete()
            except Exception:
                # Если удалить не получилось (например, сообщение слишком
                # старое) — просто оставляем его и отправляем новое
                pass
...
async def cmd_start(message: Message):
    """Обработчик команды /start — единый стартовый экран (фото + текст + кнопка)."""
    user_id = message.from_user.id
    # Сбрасываем состояние пользователя на старте (на случай если был старый прогресс)
    user_states.pop(user_id, None)

    await send_welcome_screen(message.chat.id)


@dp.message(Command("restart"))
async def cmd_restart(message: Message):
    """Обработчик команды /restart — сбрасывает прогресс и показывает стартовый экран заново."""
    user_id = message.from_user.id
    user_states.pop(user_id, None)
...
async def callback_start_test(callback: CallbackQuery):
    ...
    # Считаем количество стартов теста
    analytics["started"] += 1
...
async def callback_restart_test(callback: CallbackQuery):
    ...
    await send_welcome_screen(callback.message.chat.id)
    await callback.answer()
